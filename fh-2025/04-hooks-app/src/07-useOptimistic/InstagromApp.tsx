...
export const InstagromApp = () => {
  ...
  const handleAddComment = async (formData: FormData) => {
    const messsageText = formData.get("post-message") as string;
    if (messsageText.trim().length === 0) return;
    console.log("Nuevo comentario: ", messsageText);

    addOptimisticComment(messsageText);
    startTransition(async () => {
      ...
      // setComments([
      //   ...comments,
      //   { id: new Date().getTime(), text: messsageText },
      // ]);

      // Codigo que hace que falle el posteo del comentario
